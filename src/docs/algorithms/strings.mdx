...
```js title=scripts/first-unique-char lineNumbers=true
...
Given two strings, return true if those strings are anagrams of eachother.

```js title=scripts/anagram-checker lineNumbers=true
const isValidAnagram = (str1, str2) => {
  ...
  for (let char of str1) {
    charLookup[char] = charLookup[char] + 1 || 1;
  }

  for (let char of str2) {
    if (!charLookup[char]) return false;
    charLookup[char] -= 1;
  }
